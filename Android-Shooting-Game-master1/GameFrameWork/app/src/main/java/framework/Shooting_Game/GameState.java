...
public class GameState implements IState {
    ...
    //보조비행기 추가
    private SubPlayer1 m_subplayer1;
    private SubPlayer2 m_subplayer2;


    private BackGround m_background;
    private ArrayList<Enemy> enemies = new ArrayList<>();
  //  private Boss1 m_boss=new Boss1(500,2000);

    public ArrayList<Missile_Player> missile_players = new ArrayList<>();
    public ArrayList<Missile_SubPlayer> missile_subplayers = new ArrayList<>();
    public ArrayList<Missile_Enemy> missile_Enemys = new ArrayList<>();
  //  public ArrayList<Missile_Boss> missile_Boss=new ArrayList<>();
    //public ArrayList<Missile_SubPlayer1>missile_subPlayer1s=new ArrayList<>();
    ...
    long LastRegenBoss=System.currentTimeMillis();
    ...
    public void Init() {
        ...
        //보조비행기
        m_subplayer1 = new SubPlayer1(AppManager.getInstance().getBitmap(R.drawable.subplayer));
        m_subplayer2 = new SubPlayer2(AppManager.getInstance().getBitmap(R.drawable.subplayer));
    ...
    public void Destroy() {
        ...
        if (m_subplayer1.state == SubPlayer1.DEAD) {
            mediaPlayer.stop();
            AppManager.getInstance().setScore(score);
            AppManager.getInstance().getMainAct().finish();

        }
        if (m_subplayer2.state == SubPlayer2.DEAD) {
            mediaPlayer.stop();
            AppManager.getInstance().setScore(score);
            AppManager.getInstance().getMainAct().finish();
    ...
    public void Update() {
        ...
        m_subplayer1.Update(gameTime);
        m_subplayer2.Update(gameTime);

        //보조비행기
        //m_subplayer.Update(gameTime);
        ...
        if (System.currentTimeMillis() - LastShootMissile >= m_player.getShootSpeed()) {
            ...
            if(score>5) {
                missile_subplayers.add(new Missile_SubPlayer(m_subplayer1.getX() + m_subplayer1.getBitmapWidth() / 6 / 6, m_subplayer1.getY()));
                missile_subplayers.add(new Missile_SubPlayer(m_subplayer2.getX() + m_subplayer2.getBitmapWidth() / 6 / 6, m_subplayer2.getY()));
            }
        ...
        for (int i = missile_players.size() - 1; i >= 0; i--) {
            ...
            //화면 밖을 벗어나면 해당 객체의 state 변수 값을 state out으로 변경
        }
        // 점수가 5점 이상일 경우 서브 비행기에서 미사일이 발사
        if(score>5) {
            for (int i = missile_subplayers.size() - 1; i >= 0; i--) {
                Missile_SubPlayer spms = missile_subplayers.get(i);
                spms.Update();
                if (spms.state == Missile.STATE_OUT) missile_subplayers.remove(i);
                //화면 밖을 벗어나면 해당 객체의 state 변수 값을 state out으로 변경
            }
        }
      ...
      /*  if(score>5){
            Boss1 bos =m_boss;
            for(int i=missile_Boss.size()-1; i>=0; i--){
                Missile_Boss mboss=missile_Boss.get(i);
                mboss.Update();
                if (mboss.state == Missile.STATE_OUT) missile_Boss.remove(i);
            }
            //bos.Update(gameTime);
            if(bos.state==Boss1.STATE_OUT) Destroy();
        }*/
        //적군 미사일
    ...
    public void Render(Canvas canvas) {
        ...
        for (Missile_SubPlayer spms : missile_subplayers) spms.Draw(canvas);

        //적군 미사일 그리기
        for (Missile_Enemy enemms : missile_Enemys) enemms.Draw(canvas);

        //보스 미사일 그리기
      //  for(Missile_Boss mboss : missile_Boss) mboss.Draw(canvas);
        ...
        m_subplayer1.Draw(canvas);
        m_subplayer2.Draw(canvas);

        //m_subplayer.Draw(canvas);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getX() > m_player.getX()) {
            ...
            m_subplayer1.setPosition(m_subplayer1.getX() + 20, m_subplayer1.getY());
            m_subplayer2.setPosition(m_subplayer2.getX() + 20, m_subplayer2.getY());
        }
        if (event.getX() < m_player.getX()) {
            m_player.setPosition(m_player.getX() - 20, m_player.getY());
            m_subplayer1.setPosition(m_subplayer1.getX() - 20, m_subplayer1.getY());
    ...
    public void MakeEnemy() {
       /* if(score>3){
            //LastRegenBoss=System.currentTimeMillis();
            //if(System.currentTimeMillis() - LastRegenBoss>=1000) {
            //LastRegenBoss = System.currentTimeMillis();

            //Boss boss = null;
            //boss = new Boss1(500,1500);
            m_boss.setPosition(randBoss.nextInt(AppManager.getInstance().getDeviceSize().x), -60);
            m_boss.movetype = randBoss.nextInt(3);
            //m_boss.add(boss);
    ...
    public void CheckCollision() {
        ...
        /*for(int i=missile_players.size()-1; i>=0; i--) {
            if (CollisionManager.CheckBoxToBox(missile_players.get(i).m_BoundBox, m_boss.m_BoundBox)) {
                soundManager.play(randEnem.nextInt(2));
                missile_players.remove(i);
                if (m_boss.getHp() > 0) {
                    m_boss.setHp(m_boss.getHp() - missile_players.get(i).getDamage());
                } else {
                    //effectExplosions.add(new EffectExplosion(m_boss.getX(), m_boss.getY()));
                    Destroy();
                    score++;
                    return;
                }
        ...
        for (int i = enemies.size() - 1; i >= 0; i--) {
            if (CollisionManager.CheckBoxToBox(enemies.get(i).m_BoundBox, m_player.m_BoundBox)) {
                ...
                if (m_player.getLife() <= 0) {
                    ...
                    m_subplayer1.state = SubPlayer1 .DEAD;
                    m_subplayer2.state = SubPlayer2.DEAD;
                }
                return;
            }
        }
        ...
        //서브 비행기 미사일과 적의 충돌 체크
        for (int i = missile_subplayers.size() - 1; i >= 0; i--) {
            for (int j = enemies.size() - 1; j >= 0; j--) {
                if (CollisionManager.CheckBoxToBox(missile_subplayers.get(i).m_BoundBox, enemies.get(j).m_BoundBox)) {
                    soundManager.play(randEnem.nextInt(2));
                    missile_subplayers.remove(i);
                    if (enemies.get(j).getHp() > 0) {
                        enemies.get(j).setHp(enemies.get(j).getHp() - missile_subplayers.get(i).getDamage());
